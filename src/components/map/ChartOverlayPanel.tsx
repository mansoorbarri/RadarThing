...
export function ChartSidePanel({ icao, onClose }: ChartSidePanelProps) {
  ...
  const allCharts = useMemo(() => {
    ...
    return CHART_TYPES.flatMap((type) =>
      chartsByType[type.key].map((chart, index) => ({
        chart,
        index,
        type: type.key,
  ...
  const searchResults = useMemo(() => {
    ...
    return allCharts.filter(({ chart, type, typeLabel }) =>
      [chart.chartName, type, typeLabel].some((value) =>
              ...
              <div className="mb-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-white/10 bg-white/5 py-1.5 pr-8 pl-8 text-[11px] text-white/80 outline-none placeholder:text-white/30 focus:border-cyan-500/30"
                  />
                  {searchQuery && (
                    <button
            ...
            {searchQuery.trim() ? (
              <div className="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-white/10 bg-black/20 p-1">
                {searchResults.length > 0 ? (
                  ...
                  <div className="px-2.5 py-3 text-center text-[11px] text-white/35">
                    No matching charts
                  </div>
                )}
              </div>

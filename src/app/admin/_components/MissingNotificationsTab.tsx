...
function matchesNotifSearch(notification: NotifType, query: string) {
  ...
  return (
    notification.airlineCode?.toLowerCase().includes(q) ||
    notification.aircraftType?.toLowerCase().includes(q) ||
    notification.note?.toLowerCase().includes(q)
  );
}
...
function EditableNote({
  ...
  return (
    <input
      ...
      className={`w-full rounded-lg border bg-transparent px-3 py-1.5 text-sm transition-all outline-none ${
        isEditing
...
export function MissingNotificationsTab() {
  ...
  const updateNoteMutation = useMutation(
    api.missingImageNotifications.updateNote,
  );
  const removeNotificationMutation = useMutation(
    api.missingImageNotifications.remove,
  );
  const purgeInvalidMutation = useMutation(
    api.missingImageNotifications.purgeInvalid,
  );
  ...
  useEffect(() => {
    if (notifAirlineFilter && !uniqueNotifAirlines.includes(notifAirlineFilter))
  ...
  useEffect(() => {
    if (
      notifAircraftFilter &&
      !uniqueNotifAircraftTypes.includes(notifAircraftFilter)
    )
      setNotifAircraftFilter("");
  }, [uniqueNotifAircraftTypes, notifAircraftFilter]);

  const hasActiveNotifFilters =
    notifSearchQuery || notifAirlineFilter || notifAircraftFilter;

  const filteredNotifications = useMemo(() => {
    return notifications
      .filter((n) => {
        if (notifAirlineFilter && n.airlineCode !== notifAirlineFilter)
          return false;
        if (notifAircraftFilter && n.aircraftType !== notifAircraftFilter)
          return false;
  ...
  const handlePurgeConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        `Purged ${result.deletedCount} invalid notification${result.deletedCount === 1 ? "" : "s"}`,
      );
    } catch {
    ...
    <>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex gap-3">
          <select
            value={notifAirlineFilter}
            onChange={(e) => setNotifAirlineFilter(e.target.value)}
          ...
          >
            ...
            {uniqueNotifAirlines.map((airline) => (
          ...
          <select
            value={notifAircraftFilter}
            onChange={(e) => setNotifAircraftFilter(e.target.value)}
            className="rounded-lg border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white transition-all outline-none focus:border-cyan-500/50"
          >
            <option value="">All Aircraft</option>
            {uniqueNotifAircraftTypes.map((type) => (
        ...
        <div className="rounded-2xl border border-white/10 bg-black/40 p-12 text-center backdrop-blur-xl">
          ...
          <p className="text-slate-400">
            {hasActiveNotifFilters
              ...
              : "No missing image notifications have been sent yet"}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                <th className="px-6 py-4 text-left font-mono text-xs font-medium tracking-wider text-slate-400 uppercase">
                  Airline Code
                </th>
                <th className="px-6 py-4 text-left font-mono text-xs font-medium tracking-wider text-slate-400 uppercase">
                  Aircraft Type
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <button
                      ...
                      className="cursor-pointer rounded-lg p-1.5 text-slate-600 opacity-0 transition-all group-hover:opacity-100 hover:bg-red-500/20 hover:text-red-400"
                    >
